...
public class ProfileFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (etBirth != null) {
            etBirth.setText(userBirthday);
            updateAgeDisplay(userBirthday);
        }
        ...
        etPhone.setOnClickListener(v -> {
            if (isEditMode && currentUser != null && currentUser.isPhoneVerified) {
                showPhoneChangePrompt();
            }
        });
    ...
    private void showChangePasswordDialog() {
        ...
        builder.setPositiveButton("Change", null); // Set to null first to override later
        builder.setNegativeButton("Cancel", null);
        
        AlertDialog dialog = builder.create();
        dialog.show();

        // Override the listener after show() to prevent auto-closing on validation error
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            ...
            if (newPass.isEmpty()) {
                tilNew.setError("Password cannot be empty");
            } else if (!isStrongPassword(newPass)) {
                tilNew.setError("Password is too weak");
            } else if (!newPass.equals(confirmPass)) {
                tilConfirm.setError("Passwords do not match");
            } else if (currentUser != null && SecurityUtils.hashPassword(newPass).equals(currentUser.password)) {
                tilNew.setError("You cannot use your current password");
            } else {
                // All validations passed, close dialog and start verification
                dialog.dismiss();
    ...
    private void updateUIForMode() {
        ...
        // Email is clickable only in edit mode; always disabled for direct typing
    ...
    private void saveProfileChanges() {
        ...
        if (!currentUser.isPhoneVerified && !newPhone.equals(currentUser.phone)) {
            ...
            startPhoneVerification(newPhone, () -> performUpdate(newName, currentUser.email, newPhone, newBirth, true));
        } else {
            performUpdate(newName, currentUser.email, currentUser.phone, newBirth, false);
        }
    }
    ...
    private void showNewPhoneInputDialog() {
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            ...
            if (currentUser != null && newPhone.equals(currentUser.phone)) {
                til.setError("You cannot use your current phone number");
                return;
            }
            if (!newPhone.matches("^(09|\\+639)\\d{9}$")) {
                til.setError("Invalid PH phone number format");
                return;
            }
    ...
    private void showNewEmailInputDialog() {
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            ...
            if (currentUser != null && newEmail.equalsIgnoreCase(currentUser.email)) {
                til.setError("You cannot use your current email");
                return;
            }
                ...
                if (getActivity() != null) {
                    getActivity().runOnUiThread(() -> {
                        if (existingUser != null) {
                            til.setError("Email already in use");
                        } else {
                            dialog.dismiss();
                            startEmailVerification(newEmail, "Verify New Email", "Enter code sent to", () -> {
                                ...
                                userEmail = newEmail;
                                if (getArguments() != null) {
                                    getArguments().putString("email", newEmail);
                                }
        ...
        EmailHelper.sendVerificationCode(email, code, (success, error) -> {
            if (getActivity() != null) {
                getActivity().runOnUiThread(() -> {
                    ...
                    if (success) {
                        showCodeInputDialog(title, messagePrefix + " " + email, code, onSuccess);
                    } else {
            ...
            if (getActivity() != null) {
                getActivity().runOnUiThread(() -> {
                    ...
                    if (success) {
                        ...
                        showCodeInputDialog("Verify Phone Number", 
